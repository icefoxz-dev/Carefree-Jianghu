...
namespace EnhancedHierarchy.Icons {
    public sealed class MonoBehaviourIcon : IconBase {
        ...
        private const string UnityEngineEventSystemsNamespace = "UnityEngine.EventSystems";
        ...
        public override void Init() {
            ...
            foreach (var c in components)
                if (c.component is MonoBehaviour) {
                    hasMonoBehaviour = true;
                    break;
                }
        }

        private static IEnumerable<(Type, Component component)> GetHierarchyWithUGuiFiltered()
        {
            return EnhancedHierarchy.Components
                .Select(c => (c.GetType(), c))
                .Where(t =>
                {
                    var namespance = t.Item1.Namespace;
                    return namespance is not null && !namespance.StartsWith(UnityEngineNamespace);
                });
        }
        ...
        public override void DoGUI(Rect rect) {
            ...
            if (Utility.ShouldCalculateTooltipAt(rect) && Preferences.Tooltips) {
                ...
                foreach (var c in components)
                    if (c.component is MonoBehaviour)
                        goComponents.AppendLine(GetComponentName(c.component));
