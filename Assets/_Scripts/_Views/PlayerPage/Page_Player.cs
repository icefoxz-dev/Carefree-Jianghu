...
public class Page_Player : PageUiBase
{
    ...
    public Page_Player(IView v, bool display = false) : base(v, display)
    {
        ...
        view_backpack = new View_backpack(v.Get<View>("view_backpack"), index => UiManager.ShowInfo("工作在进行中！"));
    ...
    private class View_backpack : UiBase
    {
        ...
        public void ShowInventory(ITagManager tm)
        {
            Show();
            var list = tm.Tags.Select(t => (default(Sprite), t.Name)).ToArray();
            SetList(list);
        }
